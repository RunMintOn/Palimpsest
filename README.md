...
2. **实时检索**：你在写作时停笔约 800ms，插件会把你当前写的那段话也转成向量，然后在索引中做相似度搜索——找到与之语义最接近的旧片段。
3. **展示结果**：匹配到的片段按相似度排序显示在右侧边栏。点击可打开原文，拖动可插入链接或引用。

整个过程全部在本地完成，不经过云端。

第一次建库的速度取决于你的 vault 大小。如果笔记较多，全量索引可能需要几分钟。这是一次性成本——之后新增和修改的笔记会自动增量更新，不需要手动重建。移动文件或文件夹会复用已有向量；改文件名、标题或正文时会更新受影响的向量。
...
- **实时语义检索**：停笔约 800ms 后，根据当前段落和标题自动检索知识库
